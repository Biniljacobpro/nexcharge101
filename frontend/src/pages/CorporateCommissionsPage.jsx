...
const CorporateCommissionsPage = ({ corporateId: corporateIdProp }) => {
  ...
  }, [corporateId, statusFilter, yearFilter, monthFilter, page]);
  ...
  const loadStats = async () => {
    try {
      const response = await api.getCommissionStats('corporate', corporateId);
      if (response.success) {
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="subtitle1" sx={{ color: '#636e72' }}>
          Track and manage commission earnings from corporate bookings
        </Typography>
      </Box>
